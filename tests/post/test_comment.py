...
class TestComment:
    ...
    def test_create_comment(self, api_client, users, posts, comment_data):
        response = api_client.post('/posts/comments', comment_data)
        assert response.status_code == 201

    def test_create_comment_without_post_no(self, api_client, users, bad_comment_data):
        response = api_client.post('/posts/comments', bad_comment_data)
    ...
    def test_detail_comments(self, api_client, comments, dummy_id):
        response = api_client.get(f'/posts/comments/{dummy_id}')
    ...
    def test_update_comment(self, api_client, comments, dummy_id, content_data):
        before_update = api_client.get(f'/posts/comments/{dummy_id}')
        response = api_client.put(f'/posts/comments/{dummy_id}', data=content_data)
    ...
    def test_update_comment_with_bad_access(self, api_client, comments, dummy_id, bad_post_data):
        response = api_client.put(f'/posts/comments/{dummy_id}', data=bad_post_data)
        assert response.status_code == 403

    def test_update_parent_comment(self, api_client, comments, dummy_id):
        response = api_client.put(f'/posts/comments/{dummy_id}', data={"parent_comment": 65535})
        assert response.status_code == 403

    def test_delete_post(self, api_client, comments, dummy_id):
        before_delete = api_client.get(f'/posts/comments/{dummy_id}')
        response = api_client.delete(f'/posts/comments/{dummy_id}')
        after_delete = api_client.get(f'/posts/comments/{dummy_id}')
